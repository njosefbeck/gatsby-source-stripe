...
function addLocalFiles(node, fileNodeIds) {
  ...
  const nodeWithLocalFiles = Object.assign(node, { localFiles___NODE: rootFileNodeIds })
  ...
  nonRootFileNodeIds.forEach(fileNodeId => {
    const localFiles = nodeWithLocalFiles[fileNodeId.location].localFiles___NODE;
    if (localFiles) {
      nodeWithLocalFiles[fileNodeId.location].localFiles___NODE.concat(fileNodeId.id)
    }
